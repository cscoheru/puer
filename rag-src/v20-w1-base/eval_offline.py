...
def _score_recall(case: dict, img: str) -> dict:
    ...
    return {
        ...
        # F1: candidates with via (sibling: presence) + the wider pool trace.
        # Cheap additive fields; downstream scorers can ignore them.
        "candidates": [{"skuId": c.get("skuId"), "skuIds": c.get("skuIds"),
                        "name": c.get("name"), "dino_sim": c.get("dino_sim"),
                        "via": c.get("via", "")} for c in cands],
        "pool": (out.get("trace") or {}).get("pool"),
    }
...
def _score_verdict(case: dict, img: str) -> dict:
    ...
    return {
        ...
        "best": ({"skuId": best.get("skuId"), "name": best.get("name"),
                  ...
                  "via": best.get("via", ""), "dino_sim": best.get("dino_sim"),
                  "notes": best.get("notes", ""),
                  "_b2_demoted": best.get("_b2_demoted", False),
                  "_e2_promoted": best.get("_e2_promoted", False),
                  "_e2_from": best.get("_e2_from")}
                 if best else None),
        "verdicts": [{"skuId": v.get("skuId"), "verdict": v.get("verdict"),
                      "confidence": v.get("confidence"),
                      "notes": v.get("notes", ""), "via": v.get("via", ""),
                      "dino_sim": v.get("dino_sim")}
                     for v in verdicts],
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        for i, case in enumerate(cases):
            ...
            record = {"case_id": cid, "category": case.get("category"),
                      ...
                      "query_skuId": case.get("query_skuId"),
