...
def extract(path):
    ...
    try:
        # USER 2026-08-25: max 2 OCR attempts, skip bailian (no token / wasted
        # 10-20s). Lead stays kimi (or RAG_OCR_PROVIDER). GLM stays out.
        # Attempt 0 demands readable TEXT; attempt 1 accepts features-only.
        import teacher_api
        lead = OCR_PROVIDER or "kimi"
        order = [p for p in [lead, lead] if p in teacher_api.PROVIDERS][:2]
        for i, prov in enumerate(order):
            ...
            if parsed and _is_useful(parsed, require_text=(i == 0)):
                return {
