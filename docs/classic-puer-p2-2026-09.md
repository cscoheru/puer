...
## R23 · 茶品库对用户绝对隐藏（2026-09-13 已上线，release 20260912T153945Z-45c410d）

**需求（安全）**：经典普洱点进茶品详情页，面包屑「品茶论坛 / 经典普洱 / **茶品库** / 茶名」暴露了茶品库入口，点击可浏览**任何**茶品（含未发布私人档案）。要求：茶品库对用户绝对隐藏，并排查全站其他暴露路径。

**排查结果（全站 grep `href="/tea"` + `茶品库` + API 调用方）**：共 7 处暴露面——详情页面包屑、品鉴详情页面包屑、主页「茶品百科」按钮、`/tea` 列表页本身（公开全量）、`/api/teas` GET（公开全量）、`/api/teas/[id]` GET（任意 id 可读 JSON）、sitemap（全量茶详情页进搜索引擎）。

**修复**：
- **UI 入口清零**：茶品详情页面包屑去掉「茶品库」（品茶论坛 / 经典普洱(若有) / 茶名）；品鉴详情页面包屑去掉「茶品库」；主页 hero「茶品百科」按钮改为「经典普洱」→ /forum/classics；
- **页面访问控制**：`/tea` 列表页非 admin → 404；`/tea/[id]` 非经典茶档案非 admin → 404（**经典茶详情页保持公开**——classics 流量承接页，R22 已有内容锁定）；
- **API 访问控制**：`/api/teas` GET 列表模式（无 q 参数）非 admin → 403（admin 管理页 fetch `?limit=9999` 不受影响）；带 q 搜索模式（发品鉴笔记/发帖选茶）保持可用但非 admin 强制 limit≤20 防枚举；`/api/teas/[id]` 非经典茶非 admin → 404；
- **SEO**：sitemap 茶品页只收录 `isClassic=true`（1973 全量 → 152 经典茶）。

**部署**：commit `45c410d`；无 migration；rsync 3.6M；build DONE；activate 正常。守卫（未登录实测）：`/tea` 404、非经典茶详情 404、经典茶详情 200、forum/classics 200、API 无 q 403 / 有 q 200 / 非经典 id 404 / 经典 id 200、详情页 HTML 中 `href="/tea"` 与「茶品库」均为 0、sitemap 仅 152 经典茶 ✓。清理旧镜像。

**关联遗留**（付费会员体系规划时一并处理）：/tasting 品鉴 feed 公开列出品鉴笔记（含茶名）是否需要会员门槛；未登录访问主页 307 行为确认。
