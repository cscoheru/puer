...
# auto-post-cron.sh.
#
# Usage: cron-task.sh <auto-reply|auto-vote|auto-boost-new|auto-post|minimax-health>
...
TASK="${1:?usage: cron-task.sh <auto-reply|auto-vote|auto-boost-new|auto-post|minimax-health>}"
...
case "$TASK" in
  minimax-health)
    # R28e: MiniMax 余额/API 健康探针 — 不需要 DB,只调 API 一次
    if [ -z "${MINIMAX}" ]; then
      log_alert "missing MINIMAX_API_KEY"
      exit 1
    fi
    export MINIMAX_API_KEY="${MINIMAX}"
    # minimax-health-check 自己处理 ok/alert 状态;非零退出码视为探针异常
    node scripts/minimax-health-check.mjs || log_alert "minimax-health probe exited $?"
    ;;
  *)
